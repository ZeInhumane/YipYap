...
module.exports = {
    async execute(message, winner, loser, location) {
        ...
        let moneyEarned = Math.floor(loser.level * location.Rewards.GoldMultiplier);
        ...
        if (Math.random() < 0.2) {
            let dropInfo = Object.values(lootboxInfo);
            let dropNames = Object.keys(lootboxInfo);
            let totalChance = 0
        ...
        await userEffects.findOne({ userID: winner.userID }, async (err, effects) => {
            await User.findOne({ userID: winner.userID }, async (err, user) => {
                ...
                if (effects != null) {
                    let goldTicketName = Object.keys(effects.tickets).filter(key => key.includes('Gold'))
