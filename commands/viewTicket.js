        ...
        User.findOne({ userID: message.author.id }, async (err, user) => {
            ...
            userEffects.findOne({ userID: message.author.id }, (err, effects) => {
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setColor('#000000')
                let ticketTypes = ['Experience', 'Gold']

                if (Object.keys(effects.tickets).length > 0) {
                    for (i = 0; i < ticketTypes.length; i++) {
                        let ticketName = Object.keys(effects.tickets).filter(key => key.includes(ticketTypes[i]))
                            ...
                            if (today >= ticket.endTime) {
                                if (ticket.auto == 'true') {
                                    if (user.inv[ticket]) {
                                        ...
                                        let end = endDate.getFullYear() + '/' + (endDate.getMonth() + 1) + '/' + endDate.getDate() + '/  ' + endDate.getHours() + ':' + endDate.getMinutes()
