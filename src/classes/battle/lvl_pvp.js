module.exports = {
    execute(message, winner, loser) {
        ...
        if (winner.level > loser.level) {
            ...
            const winner_higher = true;
        } else {
            const lvl_diff = loser.level - winner.level;
            const winner_higher = false;
        }
        ...
        const winner_exp = winning_exp(winner_higher, lvl_diff);
        const loser_exp = losing_exp(winner_higher, lvl_diff);
        ...
        const update_winner = new is_lvlup(winner.exp, winner.level, winner.userID);
        const update_loser = new is_lvlup(loser.exp, loser.level, loser.userID);
