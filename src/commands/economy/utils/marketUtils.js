exports.returnMessage = ({ listing, type, info, messageType, itemName, itemQuantity, itemPrice }) => {
    ...
    switch (type) {
        ...
        case "equipment":
            switch (messageType) {
                case "confirmRemove":
                    return `Are you sure you want to remove ${listing?.quantity === 1 ? '' : `**${listing?.quantity}** of`} your **${info?.rarity}** Level __${info?.level}__ **${listing?.itemName}** [Ascension: ${info?.ascension}] that you listed on the market for **${listing?.itemCost}** Gold <:cash_24:751784973488357457>${listing?.quantity === 1 ? '' : " **each** "}? (ID: ${listing?.listingID})`

                case "listing":
                    return `**${listing?.itemCost * listing?.quantity} Gold** <:cash_24:751784973488357457>** | ${listing?.quantity === 1 ? listing?.itemName : listing?.itemName + 's'}[Asc ${info?.ascension}]**\n${listing?.item.rarity} | Level ${listing?.item.level} | ID: ${listing?.listingID}`

                case "listingConfirmed":
                    return `You have successfully placed your **${info?.rarity}** Level __${info?.level}__ **${listing?.itemName}** [Ascension: ${info?.ascension}] in the market for **${listing?.itemPrice}** Gold <:cash_24:751784973488357457>!`
