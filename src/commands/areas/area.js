...
module.exports = {
    ...
    async execute({ client, user, message, args, prefix }) {
        if (!args[0]) {
        ...
        const input = Number.isNaN(parseInt(args[0]))
            ? { type: "name", value: args[0] }
            : { type: "id", value: parseInt(args[0]) };
        ...
        user.save()
            .then((result) =>
                console.log(
                    `Set area for ${result._doc.userID} to ${result._doc.location.area}`,
                ),
            )
...
function getArea(input, message, prefix) {
    if (input.type === "name") {
        ...
        message.channel.send(
            `You need to specify a valid area name.\n` +
    ...
    if (input.type === "id") {
        ...
        message.channel.send(
            `You need to specify a valid area id.\n` +
                `Correct usage: \`${prefix}area ${this.syntax}\``,
        );
        return;
    }
