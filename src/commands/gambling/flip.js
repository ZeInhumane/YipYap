...
module.exports = {
    ...
    execute({ message, args }) {
        User.findOne({ userID: message.author.id }, async (err, user) => {
            ...
            // Finds faceBetted
            const faceBettedIndex = args.findIndex(arg => /^(heads?|tails?)$/gi.test(arg));
            ...
            const faceBetted = args[faceBettedIndex].toLowerCase().replace(/s$/g, "");

            const rng = Math.floor(Math.random() * 10002);
            ...
            if (rng == 5000) {
                ...
                const isWin = faceBetted == "head";
                win(isWin);
                faceLanded = "on heads <:CoinHead:753518321214816297>.";
            } else {
                const isWin = faceBetted == "tail";
                win(isWin);
            ...
            user.save()
                .then(result => console.log("flip", result))
